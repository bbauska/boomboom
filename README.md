...
for ((i=5; i<=10; ++i)) ; do printf '%02dn' $i ; done  
...
du |   
sort -r -n |   
awk '{split("K M G",v); s=1; while($1>1024){$1/=1024; s++} print int($1)" "v[s]"t"$2}' |   
head -n 20  
```
...
 printf "[e[1;32m+e[m] resolve: %sn" "$_domain"
...
   printf "[e[1;32m+e[m] trace + headers: %s://%sn" "$_proto" "$_domain"
...
```bash  
server> while : ; do   
(echo -ne "HTTP/1.1 200 OKrnContent-Length: $(wc -c <index.html)rnrn" ; cat index.html;) |   
nc -l -p 5000   
; done  
```
...
printf "  lport: %snbk_host: %snbk_port: %snn"   
...
sort | uniq -c | awk '{ printf("%st%st",$2,$1) ; for (i = 0; i < $1; i++) {printf("*")}; print "" }'  
```
...
```bash  
awk '{ print FNR "t" $0 }' filename  
awk '{ printf("%5d : %sn", NR, $0) }' filename   # in a fancy manner  
```
...
```bash  
awk -F' ' '{print "ip:t" $2 "n port:t" $3' filename  
```
...
```bash  
sed ':a;N;$!ba;s/n/ /g' /path/to/file

# cross-platform compatible syntax:  
sed -e ':a' -e 'N' -e '$!ba' -e 's/n/ /g' /path/to/file  
```
...
```bash  
perl -p -i.orig -e 's/bfoob/bar/g' *.conf  
```
...
   echo -en "Unsuccessful domain name resolution.n"

 else

   echo -en "$_host > $_host_ipn"
...
   echo -en "Unsuccessful ASN gathering.n"
